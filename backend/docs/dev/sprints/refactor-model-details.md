...
* **Cálculo Dinámico de Variables Temporales:** Se eliminaron los campos estáticos `lifetime` y `monthToEndContract`. El Backend ahora los calcula dinámicamente en el `ClientPredictionMapper` usando `ChronoUnit.MONTHS` entre la fecha de registro y la fecha actual.
...
* **Lógica Analítica en el Mapper:** Se introdujo lógica de *Java Streams* para agrupar cargos por `YearMonth`. Esto permite calcular el promedio mensual de gastos de forma dinámica (`sumOfMonthlyAverages`), asegurando que la IA reciba tendencias actualizadas del comportamiento del cliente.

### 3. Categorización y Tipificación (ChargeType)

* **Normalización de Gastos:** Se implementó la entidad `ChargeType` para clasificar la naturaleza de los cargos adicionales.
...
* **Catálogo Inicial:** El sistema inicia con categorías predefinidas como 'Massage Therapy' y 'Dietary supplements', permitiendo una expansión futura sin alterar el esquema principal.
...
* **Validaciones en Capa de Servicio:** El `ClientService` ahora valida la existencia del `Partner` antes de persistir un nuevo cliente, lanzando una `EntityNotFoundException` si el ID no es válido.
